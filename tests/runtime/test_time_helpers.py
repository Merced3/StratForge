...
def test_normalize_session_times_with_pandas_timestamp():
    ...
    open_dt, close_dt = main.normalize_session_times(ts, ts + pd.Timedelta(hours=6, minutes=30))
...
def test_normalize_session_times_none_safe():
    assert main.normalize_session_times(None, None) == (None, None)
...
def test_build_candle_schedule_basic():
    ...
    timestamps, buffer_ts = main.build_candle_schedule(session_open, session_close, timeframes, durations, buffer_secs)
